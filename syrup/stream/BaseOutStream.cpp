...
BaseOutStream& operator<<(BaseOutStream& os, BaseOutStreamManipulator& manip)
{
    return manip.action(os);
}

BaseOutStream& __flush(BaseOutStream& os) { os.flush(); return os; }
BaseOutStream& __endl(BaseOutStream& os) { return os << '\n' << flush;}

BaseOutStreamManipulator flush(__flush);
BaseOutStreamManipulator endl(__endl);
